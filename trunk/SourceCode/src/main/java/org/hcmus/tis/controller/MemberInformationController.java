...
public class MemberInformationController {
    ...
    public String create(String email, Long memberRoleId, Long projectId, @RequestParam(value = "redirectUrl", required = false) String redirectUrl, HttpServletRequest httpServletRequest) {
        MemberRole memberRole = MemberRole.findMemberRole(memberRoleId);
        Project project = Project.findProject(projectId);
        Account account = Account.findAccountsByEmailEquals(email).getResultList().get(0);
    ...
    public String delete(@RequestParam(value = "redirectUrl", required = false) String redirectUrl, @PathVariable("id") Long id, @RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
    ...
    @RequestMapping(value = "/{id}", params = { "fromProjectForm", "redirectUrl" }, produces = "text/html")
    public String updateFromProjectForm(@PathVariable("id") Long id, Model uiModel, String redirectUrl) {
        MemberInformation memberInformation = MemberInformation.findMemberInformation(id);
    ...
    public String update(@RequestParam(value = "redirectUrl", required = false) String redirectUrl, @Valid MemberInformation memberInformation, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
