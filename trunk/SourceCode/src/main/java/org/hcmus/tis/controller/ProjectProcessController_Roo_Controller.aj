...
privileged aspect ProjectProcessController_Roo_Controller {
    ...
    public String ProjectProcessController.create(@Valid ProjectProcess projectProcess, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
    ...
    public String ProjectProcessController.list(@RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
        if (page != null || size != null) {
            int sizeNo = size == null ? 10 : size.intValue();
            final int firstResult = page == null ? 0 : (page.intValue() - 1) * sizeNo;
            uiModel.addAttribute("projectprocesses", ProjectProcess.findProjectProcessEntries(firstResult, sizeNo));
            float nrOfPages = (float) ProjectProcess.countProjectProcesses() / sizeNo;
    ...
    public String ProjectProcessController.updateForm(@PathVariable("id") Long id, Model uiModel) {
        populateEditForm(uiModel, ProjectProcess.findProjectProcess(id));
        return "projectprocesses/update";
    }
    
    @RequestMapping(value = "/{id}", method = RequestMethod.DELETE, produces = "text/html")
    public String ProjectProcessController.delete(@PathVariable("id") Long id, @RequestParam(value = "page", required = false) Integer page, @RequestParam(value = "size", required = false) Integer size, Model uiModel) {
        ProjectProcess projectProcess = ProjectProcess.findProjectProcess(id);
